# SpeedTest Service Setup — run as Administrator on each machine
# Prerequisites: Git, Node.js, .NET 10 SDK, NSSM, Ookla speedtest CLI
$ErrorActionPreference = "Stop"
$repoDir = "C:\Development\SpeedTest"
$publishDir = "$repoDir\SpeedTest.Api\publish"
...
dotnet publish "$repoDir\SpeedTest.Api" -c Release -o $publishDir

# Copy frontend
if (Test-Path "$publishDir\wwwroot") { Remove-Item "$publishDir\wwwroot" -Recurse -Force }
Copy-Item "$repoDir\client\dist" "$publishDir\wwwroot" -Recurse

# Install SpeedTest service
Write-Host "Installing SpeedTest service..."
& $nssm install SpeedTest "C:\Program Files\dotnet\dotnet.exe" "$publishDir\SpeedTest.Api.dll"
& $nssm set SpeedTest AppDirectory $publishDir
& $nssm set SpeedTest DisplayName "SpeedTest"
& $nssm set SpeedTest Description "ISP Speed Test Monitor"
& $nssm set SpeedTest Start SERVICE_AUTO_START
& $nssm set SpeedTest AppStdout "$repoDir\logs\service.log"
& $nssm set SpeedTest AppStderr "$repoDir\logs\service.log"
& $nssm set SpeedTest AppRotateFiles 1
& $nssm set SpeedTest AppRotateBytes 1048576
...
Start-Service SpeedTest
Start-Service SpeedTestUpdater

Write-Host ""
Write-Host "Done! SpeedTest is running on http://localhost:5091"
